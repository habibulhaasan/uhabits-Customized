...
import org.isoron.uhabits.databinding.ActivityTaskCategoryListBinding
import org.isoron.uhabits.databinding.DialogTaskCategoryEditBinding
import org.isoron.uhabits.utils.applyRootViewInsets
import org.isoron.uhabits.utils.setupToolbar

class TaskCategoryListActivity : AppCompatActivity() {

    private lateinit var binding: ActivityTaskCategoryListBinding
    private lateinit var taskCategoryList: org.isoron.uhabits.core.models.TaskCategoryList
    private lateinit var adapter: TaskCategoryAdapter
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityTaskCategoryListBinding.inflate(LayoutInflater.from(this))
        binding.root.setupToolbar(
            toolbar = binding.toolbar,
            title = getString(R.string.manage_task_categories),
        ...
        adapter = TaskCategoryAdapter(
            categories = taskCategoryList.getAll().toMutableList(),
            theme = themeSwitcher.currentTheme,
            onClick = { showEditCategoryDialog(it) },
            onDelete = { confirmDelete(it) },
            onReorder = { from, to -> taskCategoryList.reorder(from, to) },
            onStartDrag = { holder -> touchHelper.startDrag(holder) }
        )
        binding.recyclerView.layoutManager = LinearLayoutManager(this)
        binding.recyclerView.adapter = adapter

        touchHelper = ItemTouchHelper(TaskCategoryDragCallback(adapter))
    ...
    override fun onResume() {
        ...
        adapter.replaceAll(taskCategoryList.getAll())
        updateEmptyState()
    }

    override fun onCreateOptionsMenu(menu: Menu): Boolean {
        menuInflater.inflate(R.menu.task_category_list, menu)
        return true
    }

    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        if (item.itemId == R.id.actionAddTaskCategory) {
    ...
    private fun showAddCategoryDialog() {
        showNameColorDialog(
            ...
            taskCategoryList.add(TaskCategory(name = name, color = color))
            adapter.replaceAll(taskCategoryList.getAll())
            updateEmptyState()
        }
    }
    ...
    private fun showEditCategoryDialog(category: TaskCategory) {
        showNameColorDialog(
            ...
            taskCategoryList.update(category)
            adapter.replaceAll(taskCategoryList.getAll())
        }
    }

    private fun confirmDelete(category: TaskCategory) {
        AlertDialog.Builder(this)
            .setTitle(getString(R.string.delete_category))
            .setMessage(getString(R.string.delete_category_confirmation))
            .setPositiveButton(R.string.delete) { _, _ ->
                taskCategoryList.remove(category)
                adapter.replaceAll(taskCategoryList.getAll())
                updateEmptyState()
            }
    ...
    private fun showNameColorDialog(
        ...
        val dialogBinding = DialogTaskCategoryEditBinding.inflate(LayoutInflater.from(this))
        ...
        dialogBinding.colorSwatch.setOnClickListener {
            ...
            supportFragmentManager.findFragmentByTag("taskCategoryColorPicker")?.let { existing ->
                supportFragmentManager.beginTransaction().remove(existing).commitAllowingStateLoss()
            }
            picker.show(supportFragmentManager, "taskCategoryColorPicker")
        }
